...
INSERT INTO osztaly2agazat (osztalyid,agazatid)
	SELECT DISTINCT osztalyid,agazatid 
		FROM eredeti;
		
ALTER TABLE eredeti ADD ofoid int;


INSERT INTO osztaly(evfolyam,betujel,osztalyfonokid)
	SELECT REGEXP_SUBSTR(osztaly,"[0-9]*"),
		REGEXP_SUBSTR(osztaly,"[a-z][0-9a-z]*$"),
			ofoid
		FROM eredeti
		WHERE osztaly<>"-"
		GROUP BY osztaly;